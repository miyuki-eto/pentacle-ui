...
export default function SunburstGraph({type, colorScheme}) {
    ...
    useEffect(() => {
        ...
        const setData = async () => {
            ...
            setCategories(data.children.map(({name}) => name));
            setJsonData(data);
            ...
            svg.style.maxHeight = `calc(100vh - (${
                (headerRef.current &&
    ...
    const button = (label) => {
        ...
        return (
            <button
                ...
                onClick={() => {
                    document.querySelector('circle').dispatchEvent(new Event('click'));
                    setTimeout(() => {
    ...
    return (
        <>
            <header ref={headerRef}>
                <article className="header-content flex-group space-between flex-align-center">
                    <div>
                        <HeaderSimple/>
                    </div>
                    <div className="flex">
                        <div className={'field inline'}>
                            <label>Search</label>
                            <input id="search" type="search" onChange={(e) => filter(e.target.value)}/>
                        </div>
                    </div>
                    {}
                </article>
            ...
            <main>
                ...
                {loading && <Spinner/>}
                <article className={"sunburst margin-top-1"} ref={diagramRef}/>
            </main>
        </>
    );
}
